...
import { simulatorEngine, METERS } from './kiotSimulator';
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    if (d.match && method === 'POST') {
      ...
      try {
        const { generateIncidentDiagnosis } = await import('./geminiService');
    ...
    if (path === '/api/ai/copilot' && method === 'POST') {
      ...
      try {
        const { askEnergyCopilot } = await import('./geminiService');
        const reply = await askEnergyCopilot(body.query, { meters: simulatorEngine.getAllLatestReadings(), incidents: simulatorEngine.getIncidents(), plantSummary: simulatorEngine.getPlantSummary() });
